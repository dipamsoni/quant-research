...
def _make_item(
    ...
    headline: str = "AAPL Hits Record High",
    summary: str = "Apple stock hit a record.",
    source: str = "Reuters",
...
class TestFinnhubNewsProvider:
    def test_normalize_maps_fields(self, provider: FinnhubNewsProvider) -> None:
        item = _make_item()
        article = provider._normalize(item, "AAPL")

        assert isinstance(article, NewsArticleSchema)
        assert article.title == "AAPL Hits Record High"
        assert article.content == "Apple stock hit a record."
    ...
    def test_normalize_tz_aware(self, provider: FinnhubNewsProvider) -> None:
        article = provider._normalize(_make_item(), "TSLA")
    ...
    def test_normalize_uses_summary_as_title_when_no_headline(
        ...
        article = provider._normalize(item, "MSFT")
        assert article.title == "Apple stock hit a record."
